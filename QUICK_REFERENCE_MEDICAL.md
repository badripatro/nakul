...
|---------|------|------|--------------|---------|
| **OpenNeuro** | `openneuro_loader` | Depression | `(batch, features)` | 4 |
| **SeizeIT1** | `seizeit_loader` | Seizure | `(batch, channels, time)` | 4 |
| **LIDC-IDRI** | `lidc_loader` | Lung Cancer | `(batch, D, H, W)` | 3 |
| **BUSI** | `busi_loader` | Breast Tumor | `(batch, C, H, W)` | 3 |
| **POCUS** | `pocus_loader` | Cardiac EF | `(batch, T, C, H, W)` | 3 |

---

## 💻 Code Templates

### Template 1: OpenNeuro (fMRI)
...
for features, labels, subjects in train:
    # features: connectivity matrix
    output = model(features)
```

### Template 2: SeizeIT1 (EEG-fMRI)
...
for batch in train:
    eeg = batch[0]['eeg']  # (batch, channels, time)
    fmri = batch[0]['fmri']  # (batch, features)
    labels = batch[1]  # Seizure state
```

### Template 3: LIDC-IDRI (CT)

```python
from data_loaders.lidc_loader import LIDCLoader

loader = LIDCLoader(data_path='./data/LIDC-IDRI', patch_size=(32, 64, 64))
train, val, test = loader.get_dataloaders(batch_size=8)

for patches, labels in train:
    # patches: 3D CT patches
    output = model(patches)
```

### Template 4: BUSI (Ultrasound)
...
### Template 5: POCUS (Cardiac Video)

```python
from data_loaders.pocus_loader import POCUSLoader

loader = POCUSLoader(data_path='./data/POCUS', n_frames=16)
train, val, test = loader.get_dataloaders(batch_size=8)

for sequences, labels in train:
    # sequences: video frames (batch, frames, C, H, W)
...
| LIDC-IDRI | 124 GB | 2-4 hours | 16 GB | 6 GB |
| BUSI | 180 MB | 1-2 min | 4 GB | 2 GB |
| POCUS | Variable | Variable | 8 GB | 4 GB |
